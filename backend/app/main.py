            ...
            async for message in pubsub.listen():
                if message['type'] == 'message':
                    if message['channel'] == "global_users_update":
                        # Global update received, send current global list to local client
                        redis_conn = await self.get_redis()
        ...
        while True:
            ...
            try:
                ...
                if target_id is not None and message_text:
                    # Send to target via Redis
                    await manager.send_personal_message(
                        {"type": "message", "from": client_id, "message": message_text},
                        int(target_id)
                    )
                    # We no longer send back to sender via Redis here because
                    # the frontend handles local state or we can let Redis handle it
                    # if we want absolute synchronization. 
                    # For now, let's keep it clean and only send to recipient.
